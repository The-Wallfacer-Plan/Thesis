...
\textbf{For P1.} 
In \aflgo, the seed distance to a specific target basicblock/function is calculated as the average shortest distance of basicblocks/function the seed covers. For example, in Figure~\ref{fig:call_chain}, for trace $a\rightarrow b\rightarrow c\rightarrow d\rightarrow T\rightarrow Z$, given the fact that $d(a, T)=2$ (shortest path: $a\rightarrow e\rightarrow T$), $d(b,T)=3$ (shortest path: $b\rightarrow c\rightarrow d\rightarrow T$), $d(c,T)=2$ (shortest path: $c\rightarrow d\rightarrow T$), $d(d,T)=1$ (shortest path: $d\rightarrow T$), we can see $d_s(abcdTZ)=(2+3+2+1+0)/5=1.6$. Analogously, $d_s(aeTZ)=(2+1+0)/3=1$ and $d_s(aefZ)=(2+1)/2=1.5$~\footnote{In practice, \aflgo calculates the trace distance at the \emph{basicblock} level with harmonic mean of the accumulative distance; nevertheless, the essential idea is the same.}. 
