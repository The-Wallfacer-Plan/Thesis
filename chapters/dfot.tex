...
Based on these, \aflgo solves the \emph{power scheduling problem} for directed fuzzing --- how many new seeds (termed ``energy'' in \aflgo) should be generated from the current test seed.
...
For example, the existing seed distance calculation algorithm used in \aflgo always favors shortest path that leads to the targets (see \S~\ref{subsec:motiv}), which may starve seeds that could be more easily mutated to reach the target location and further trigger crashes.
...
The first challenge is how to properly \emph{allocate} energy to the seeds with different distances and how to \emph{prioritize} the seeds closer to the targets.
